...
class HelloWorldStubAdapter
 ...
 public:
    ///Notifies all remote listeners about a change of value of the attribute HelloState.
    virtual void fireHelloStateAttributeChanged(const bool& HelloState) = 0;

    /**
    * Sends a broadcast event for HelloBroadcast. Should not be called directly.
    * Instead, the "fire<broadcastName>Event" methods of the stub should be used.
    */
    virtual void fireHelloBroadcastEvent(const std::string &_message) = 0;


    virtual void deactivateManagedInstances() = 0;

    void lockHelloStateAttribute(bool _lockAccess) {
        if (_lockAccess) {
            helloStateMutex_.lock();
        } else {
            helloStateMutex_.unlock();
        }
    }
...
protected:
    ...
    std::recursive_mutex helloStateMutex_;
};
...
public:
    ...
    /// Provides getter access to the attribute HelloState
    virtual const bool &getHelloStateAttribute(const std::shared_ptr<CommonAPI::ClientId> _client) = 0;
    /// sets attribute with the given value and propagates it to the adapter
    virtual void fireHelloStateAttributeChanged(bool _value) {
    auto stubAdapter = CommonAPI::Stub<HelloWorldStubAdapter, HelloWorldStubRemoteEvent>::stubAdapter_.lock();
    if (stubAdapter)
        stubAdapter->fireHelloStateAttributeChanged(_value);
    }
    void lockHelloStateAttribute(bool _lockAccess) {
        auto stubAdapter = CommonAPI::Stub<HelloWorldStubAdapter, HelloWorldStubRemoteEvent>::stubAdapter_.lock();
        if (stubAdapter)
            stubAdapter->lockHelloStateAttribute(_lockAccess);
    }
    ...
    /// Sends a broadcast event for HelloBroadcast.
    virtual void fireHelloBroadcastEvent(const std::string &_message) {
        auto stubAdapter = CommonAPI::Stub<HelloWorldStubAdapter, HelloWorldStubRemoteEvent>::stubAdapter_.lock();
        if (stubAdapter)
            stubAdapter->fireHelloBroadcastEvent(_message);
    }
